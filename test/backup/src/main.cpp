...
void Task_PD_Update(void *pvParameters);
...
void setup() {
  ...
  HAL::LCD_Init();
  HAL::PD_Init();
  xWireMutex = xSemaphoreCreateMutex();  // I2C 总线互斥锁
  ...
  xTaskCreatePinnedToCore(Task_PD_Update, "Task_PD_Update", 4096, NULL, 3, &xTaskPD, 0);
...
void Task_PD_Update(void *pvParameters)
{
  Serial.println(F("[PD-TASK] started"));
  TickType_t xLastWakeTime = xTaskGetTickCount();
  while (1)
  {
    HAL::PD_Update();  // PD Sniffer >= 100Hz, 10ms = 100Hz
    taskYIELD();
    vTaskDelayUntil(&xLastWakeTime, pdMS_TO_TICKS(10)); // delay for 10ms
  }
}
