...
static void _DrawMainContent() {
    ...
    if (PD.pd_connected) {
        ...
        spr.print("PD");
        if (PD.pd_is_pps) { spr.print("/PPS"); }
        spr.unloadFont();
        spr.loadFont(Font1_14);
        spr.setCursor(166,20);
        spr.setTextColor(0x07FF);
        spr.print("V: ");
        spr.print(PD.pd_voltage, PD.pd_voltage < 10 ? 3 : 2);
        spr.print("V");
        spr.setCursor(166,40);
        spr.print("I: ");
        spr.print(PD.pd_current, PD.pd_current < 10 ? 3 : 2);
        spr.print("A");
        spr.unloadFont();
    }
    else if (INA.charge_mAh <= 99999 || INA.energy_mWh <= 99999) {
        spr.loadFont(Font1_12);
        spr.setCursor(168,5);
        spr.setTextColor(0xFEA0);
        spr.print("mAh/mWh");
